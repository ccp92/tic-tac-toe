describe HumanPlayer do
  ...
  it 'does not allow a human to play in a position occupied by computer' do
    ...
    expect(HumanPlayer.new(game_state: game_state).has_turn(0)).to eq(errors: [:space_is_taken])
  end
  ...
  it 'does not allow a human to play in a position not on the board' do
    ...
    expect(HumanPlayer.new(game_state: game_state).has_turn(11)).to eq(errors: [:space_does_not_exist])
  end
